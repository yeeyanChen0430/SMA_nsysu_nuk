...
ptt_data =fread("merge_articleMetaData.csv",encoding = 'UTF-8')
...
ptt = ptt_data %>%
   filter(grepl("併校|合併|合校|公聽會",artTitle)|grepl("併校|合併|合校|公聽會",sentence)) %>%
   filter(grepl("高大|中山|高雄大學",artTitle)|grepl("高大|中山|高雄大學",sentence)) 
...
nsysu_id =nsysu %>%
  select(id)
write.csv(nsysu, file = "nsysu_id.csv")
nuk_id =nuk %>%
  select(id)
write.csv(nuk, file = "nuk_id.csv")
```
...
#編碼
```{r}
pttcomment =fread("user4.merge.csv",encoding = 'UTF-8') #818
pttcomment$artComments
